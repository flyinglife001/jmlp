...
class Solver
{
	...
	void reconstruct_gradient()
	{
		...
		if (2 * nr_free < active_size)
			svm.info("WARNING: using -h 0 may be faster\n");
...
public class svm
{
			...
			double[] alpha, Solver.SolutionInfo si, double Cp, double Cn)
	{
		...
		// if (Cp==Cn)
		// OutFile.printf("nu = %4.2f ",sum_alpha/(Cp*prob.l));
	...
	public static decision_function svm_train_one(svm_problem prob,
			svm_parameter param, double Cp, double Cn)
	{
		...
		// OutFile.printf("obj = %4.2f , rho = %4.2f ",si.obj,si.rho);
...
//		}
	...
	public static double eval_kernel(svm_node[] x, svm_node[] y, svm_parameter param)
	{
		return Kernel.k_function(x, y, param);
	}

	// jxb: modify the interface to fit the framework of the mlrap.
	public static double[] svm_predict_classify(svm_model model, svm_node[] x)
	{
		int nr_class = model.nr_class;
		...
		// OutFile.printf(vote);
		// OutFile.printf(model.label);
		// OutFile.printf("max:%d map: %d\n",vote_max_idx,model.label[vote_max_idx]);
